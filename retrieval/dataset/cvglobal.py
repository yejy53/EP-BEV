...
class CVUSADatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        self.df = pd.read_csv(f'{data_folder}/splits_csv/train.csv', header=None)
        self.df = self.df.rename(columns={0:"sat", 1:"ground"})
    ...
    def __getitem__(self, index):
        ...
        query_img = cv2.imread(f'{ground}')
        query_img = cv2.cvtColor(query_img, cv2.COLOR_BGR2RGB)

        # load reference -> satellite image
        reference_img = cv2.imread(f'{sat}')
...
class CVUSADatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if split == 'train':
            ...
            self.df = pd.read_csv(f'{data_folder}/splits/val.csv', header=None)
