...
class CVACTDatasetTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load query -> bird's eye view image
        query_img = cv2.imread(f'{self.data_folder}/ANU_data_small/bev/{idx}_grdView.jpg')
        query_img = cv2.cvtColor(query_img, cv2.COLOR_BGR2RGB)

        # Flip both reference and query images
        ...
        if self.transforms_reference is not None:
            ...
            query_img = self.transforms_reference(image=query_img)['image']  # Apply the same transformation

        # Rotate both reference and query images
...
class CVACTDatasetTest(Dataset):
    ...
    def __init__(self,
        ...
        files_ground = glob(f'{self.data_folder}/ANU_data_test/bev/*_grdView.jpg')
    ...
    def __getitem__(self, index):
        ...
        if self.img_type == "reference":
            ...
            path = f'{self.data_folder}/ANU_data_test/bev/{idx}_grdView.jpg'
